...
def load(args):
  ...
  while True:
    ...
    if feat_dim < 0:
      feat_dim = len(X)
    split, lab, text = label_l.split(None,2)
...
def classify(args, train_X, train_y, dev_X, dev_y, test_X, test_y, feat_dim, num_classes):
  classifier_config = {'nhid': args.nhid, 'optim': 'adam', 'batch_size': 64, 'tenacity': 5, 'epoch_size': 1, 'dropout': args.dropout}
  ...
  for reg in regs:
    ...
    clf.fit(train_X, train_y, validation_data=(train_X, train_y)) #after
    #scores.append(round(100*clf.score(dev_X, dev_y), 2))
    scores.append(round(100*clf.score(train_X, train_y), 2))  #after
    props.append([reg])
